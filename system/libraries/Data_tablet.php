...
$tablet_brand = $_GET['tablet_brand'] ?? '';
$tablet_model = $_GET['tablet_model'] ?? '';
$imei_number = $_GET['imei_number'] ?? '';
$tablet_cpu = $_GET['tablet_cpu'] ?? '';
$tablet_speed = $_GET['tablet_speed'] ?? '';
$tablet_ram = $_GET['tablet_ram'] ?? '';
$tablet_rom = $_GET['tablet_rom'] ?? '';
$os_tablet = $_GET['os_tablet'] ?? '';
$status_equipment = $_GET['status_equipment'] ?? '';
$department = $_GET['department'] ?? '';
$segment = $_GET['segment'] ?? '';
$division = $_GET['division'] ?? '';
$working = $_GET['working'] ?? '';
$owner_tablet = $_GET['owner_tablet'] ?? '';
...
if (!empty($serial_number)) {
    $where[] = "tl.serial_number LIKE '%" . mysqli_real_escape_string($conn, $serial_number) . "%'";
}
if (!empty($year_equipment)) {
    $where[] = "tl.year_equipment = '" . mysqli_real_escape_string($conn, $year_equipment) . "'";
}
if (!empty($tablet_brand)) {
    $where[] = "tl.tablet_brand = '" . mysqli_real_escape_string($conn, $tablet_brand) . "'";
}
if (!empty($tablet_model)) {
    $where[] = "tl.tablet_model = '" . mysqli_real_escape_string($conn, $tablet_model) . "'";
}
if (!empty($imei_number)) {
    $where[] = "tl.imei_number = '" . mysqli_real_escape_string($conn, $imei_number) . "'";
}
if (!empty($tablet_cpu)) {
    $where[] = "tl.tablet_cpu = '" . mysqli_real_escape_string($conn, $tablet_cpu) . "'";
}
if (!empty($tablet_speed)) {
    $where[] = "tl.tablet_speed = '" . mysqli_real_escape_string($conn, $tablet_speed) . "'";
}
if (!empty($tablet_ram)) {
    $where[] = "tl.tablet_ram = '" . mysqli_real_escape_string($conn, $tablet_ram) . "'";
}
if (!empty($tablet_rom)) {
    $where[] = "tl.tablet_rom = '" . mysqli_real_escape_string($conn, $tablet_rom) . "'";
}
if (!empty($os_tablet)) {
    $where[] = "os_tablet = '" . mysqli_real_escape_string($conn, $os_tablet) . "'";
}
if (!empty($status_equipment)) {
    $where[] = "tl.status_equipment = '" . mysqli_real_escape_string($conn, $status_equipment) . "'";
}
if (!empty($department)) {
    $where[] = "tl.department = '" . mysqli_real_escape_string($conn, $department) . "'";
}
if (!empty($segment)) {
    $where[] = "tl.segment = '" . mysqli_real_escape_string($conn, $segment) . "'";
}
if (!empty($division)) {
    $where[] = "tl.division = '" . mysqli_real_escape_string($conn, $division) . "'";
}
if (!empty($working)) {
    $where[] = "tl.working = '" . mysqli_real_escape_string($conn, $working) . "'";
}
if (!empty($owner_ups)) {
    $where[] = "tl.owner_tablet LIKE '%" . mysqli_real_escape_string($conn, $owner_tablet) . "%'";
}
...
$sql = "SELECT tl.id, tl.serial_number, tl.year_equipment,
        ...
        LEFT JOIN `tablet_brands` AS tb ON tl.tablet_brand = tb.id
        LEFT JOIN `tablet_models` AS tm ON tl.tablet_model = tm.id
        LEFT JOIN `tablet_cpus` AS tc ON tl.tablet_cpu = tc.id
        LEFT JOIN `tablet_speeds` AS ts ON tl.tablet_speed = ts.id
        LEFT JOIN `tablet_rams` AS ra ON tl.tablet_ram = ra.id
        LEFT JOIN `tablet_roms` AS ro ON tl.tablet_rom = ro.id
        LEFT JOIN `os_tablets` AS os ON tl.os_tablet = os.id
        LEFT JOIN `status_equipments` AS st ON tl.status_equipment = st.id
        LEFT JOIN `departments` AS d ON tl.department = d.id
        LEFT JOIN `segments` AS s ON tl.segment = s.id
        LEFT JOIN `divisions` AS v ON tl.division = v.id
        LEFT JOIN `workings` AS w ON tl.working = w.id";
...
<head>
    ...
    <title>EQUIP | Data Equipment Tablet.</title>
    <!--MATERIAL ICONS CDN-->
...
$labels = [
    ...
    'tablet_brand' => 'แบรนด์',
    'tablet_model' => 'รุ่น/โมเดล',
    'imei_number' => 'รหัสประจำเครื่อง',
    'tablet_cpu' => 'ซีพียู',
    'tablet_speed' => 'ความเร็ว',
    'tablet_ram' => 'แรม',
    'tablet_rom' => 'รอม',
    'os_tablet' => 'ระบบปฏิบัติการ',
    'status_equipment' => 'สถานะ',
    'department' => 'กอง/สำนัก',
    'segment' => 'ส่วน',
    'division' => 'ฝ่าย',
    'working' => 'งาน',
    'owner_tablet' => 'ผู้ใช้งาน',
];
...
foreach ($_GET as $key => $value) {
    if (!empty($value)) {
        ...
        switch ($key) {
            case 'tablet_brand':
                $displayValue = getNameById($conn, 'tablet_brands', $value);
                break;
            case 'tablet_model':
                $displayValue = getNameById($conn, 'tablet_models', $value);
                break;
            case 'tablet_cpu':
                $displayValue = getNameById($conn, 'tablet_cpus', $value);
                break;
            case 'tablet_speed':
                $displayValue = getNameById($conn, 'tablet_speeds', $value);
                break;
            case 'tablet_ram':
                $displayValue = getNameById($conn, 'tablet_rams', $value);
                break;
            case 'tablet_rom':
                $displayValue = getNameById($conn, 'tablet_roms', $value);
                break;
            case 'os_tablet':
                $displayValue = getNameById($conn, 'os_tablets', $value);
                break;
...
?>
                ...
                <a href="../../application/controllers/admin/search/Search_tablet.php">ค้นหาข้อมูล</a>     
            </div>
        </aside>
        <!------------------- END OF ASIDE ------------------->
        <section class="middle">
            ...
            <div class="wrapper">
                <div class="btns">
                    <a href="../../application/controllers/admin/add/Add_tablet.php">เพิ่มข้อมูลผู้ใช้งาน</a>
                    <a href="../database/download/export_tablet.php">ดาวน์โหลดข้อมูล</a>
                </div>
            </div>
            <div class="content_wrapper">
    <div id="ups_list_container" class="data_equipment_tablet">
                ...
                <tbody>
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo "<tr>
                                ...
                                <td>{$row['tablet_brand_name']}</td>
                                <td>{$row['tablet_model_name']}</td>
                                <td>{$row['imei_number']}</td>
                                <td>{$row['tablet_cpu_name']}</td>
                                <td>{$row['tablet_speed_name']}</td>
                                <td>{$row['tablet_ram_name']}</td>
                                <td>{$row['tablet_rom_name']}</td>
                                <td>{$row['os_tablet_name']}</td>
                                <td>{$row['status_equipment_name']}</td>
                                <td>{$row['department_name']}</td>
                                <td>{$row['segment_name']}</td>
                                <td>{$row['division_name']}</td>
                                <td>{$row['working_name']}</td>
                                <td>{$row['owner_tablet']}</td>
                                <td>{$row['created_at']}</td>
                                <td><a href='/Equipment-Lampang-city//application/controllers/admin/update/Update_tablet.php?id=" . $row['id'] . "'>
                                    <span class='material-icons-sharp'>drive_file_rename_outline</span>
                                </a></td>
                                <td><a href='delete_tablet.php?id=" . $row['id'] . "'>
        ...
        <section class="right">
            <div class="investments">
                <div class="header">
                    <h2>แบรนด์แทบเล็ต</h2>
                    <a href="../../application/controllers/admin/search/Search_tablet.php">เพิ่มเติม <span class="material-icons-sharp">
                            chevron_right
                        </span></a>
                </div>
