...
$scanner_brand = $_GET['scanner_brand'] ?? '';
$scanner_model = $_GET['scanner_model'] ?? '';
$printing_speed = $_GET['printing_speed'] ?? '';
$scanner_paper_size = $_GET['scanner_paper_size'] ?? '';
$status_equipment = $_GET['status_equipment'] ?? '';
$department = $_GET['department'] ?? '';
$segment = $_GET['segment'] ?? '';
$division = $_GET['division'] ?? '';
$working = $_GET['working'] ?? '';
$owner_ups = $_GET['owner_scanner'] ?? '';


$where = [];

if (!empty($serial_number)) {
    $where[] = "sl.serial_number LIKE '%" . mysqli_real_escape_string($conn, $serial_number) . "%'";
}
if (!empty($year_equipment)) {
    $where[] = "sl.year_equipment = '" . mysqli_real_escape_string($conn, $year_equipment) . "'";
}
if (!empty($scanner_brand)) {
    $where[] = "sl.scanner_brand = '" . mysqli_real_escape_string($conn, $scanner_brand) . "'";
}
if (!empty($scanner_model)) {
    $where[] = "sl.scanner_model = '" . mysqli_real_escape_string($conn, $scanner_model) . "'";
}
if (!empty($printing_speed)) {
    $where[] = "sl.printing_speed = '" . mysqli_real_escape_string($conn, $printing_speed) . "'";
}
if (!empty($scanner_paper_size)) {
    $where[] = "sl.scanner_paper_size = '" . mysqli_real_escape_string($conn, $scanner_paper_size) . "'";
}
if (!empty($status_equipment)) {
    $where[] = "sl.status_equipment = '" . mysqli_real_escape_string($conn, $status_equipment) . "'";
}
if (!empty($department)) {
    $where[] = "sl.department = '" . mysqli_real_escape_string($conn, $department) . "'";
}
if (!empty($segment)) {
    $where[] = "sl.segment = '" . mysqli_real_escape_string($conn, $segment) . "'";
}
if (!empty($division)) {
    $where[] = "sl.division = '" . mysqli_real_escape_string($conn, $division) . "'";
}
if (!empty($working)) {
    $where[] = "sl.working = '" . mysqli_real_escape_string($conn, $working) . "'";
}
if (!empty($owner_scanner)) {
    $where[] = "sl.owner_scanner LIKE '%" . mysqli_real_escape_string($conn, $owner_scanner) . "%'";
}
...
$sql = "SELECT sl.id, sl.serial_number, sl.year_equipment,
        ...
        LEFT JOIN `scanner_brands` AS sb ON sl.scanner_brand = sb.id
        LEFT JOIN `scanner_models` AS sm ON sl.scanner_model = sm.id
        LEFT JOIN `printing_speeds` AS ps ON sl.printing_speed = ps.id
        LEFT JOIN `scanner_paper_sizes` AS sp ON sl.scanner_paper_size = sp.id
        LEFT JOIN `status_equipments` AS se ON sl.status_equipment = se.id
        LEFT JOIN `departments` AS d ON sl.department = d.id
        LEFT JOIN `segments` AS s ON sl.segment = s.id
        LEFT JOIN `divisions` AS v ON sl.division = v.id
        LEFT JOIN `workings` AS w ON sl.working = w.id";
...
if (!$result) {
    ...
    echo "<pre>$sql</pre>"; // แสดงคำสั่ง SQL ที่สร้าง
    exit; // หยุดการทำงานเพื่อดู error ก่อน
}
?>
...
<head>
    ...
    <title>EQUIP | Data Equipment Scanner.</title>
    <!--MATERIAL ICONS CDN-->
...
$labels = [
    ...
    'scanner_brand' => 'แบรนด์',
    'scanner_model' => 'รุ่น/โมเดล',
    'printing_speed' => 'ความเร็วในการพิมพ์',
    'scanner_paper_size' => 'ขนาดกระดาษ',
...
foreach ($_GET as $key => $value) {
    if (!empty($value)) {
        ...
        switch ($key) {
            case 'scanner_brand':
                $displayValue = getNameById($conn, 'scanner_brands', $value);
                break;
            case 'scanner_model':
                $displayValue = getNameById($conn, 'scanner_models', $value);
                break;
            case 'printing_speed':
                $displayValue = getNameById($conn, 'printing_speeds', $value);
                break;
            case 'scanner_paper_size':
                $displayValue = getNameById($conn, 'scanner_paper_sizes', $value);
                break;
...
?>
                ...
                <a href="../../application/controllers/user/search/Search_scanner.php">ค้นหาข้อมูล</a>     
            </div>
        </aside>
        <!------------------- END OF ASIDE ------------------->
                ...
                <tbody>
                    ...
                    while ($row = mysqli_fetch_assoc($result)) {
                        echo "<tr>
                                ...
                                <td>{$row['scanner_brand_name']}</td>
                                <td>{$row['scanner_model_name']}</td>
                                <td>{$row['printing_speed_name']}</td>
                                <td>{$row['scanner_paper_size_name']}</td>
                                <td>{$row['status_equipment_name']}</td>
                                <td>{$row['department_name']}</td>
                                <td>{$row['segment_name']}</td>
                                <td>{$row['division_name']}</td>
                                <td>{$row['working_name']}</td>
                                <td>{$row['owner_scanner']}</td>
                                <td>{$row['created_at']}</td>
                                <td><a href='/Equipment-Lampang-city//application/controllers/admin/update/Update_scanner.php?id=" . $row['id'] . "'>
                                    <span class='material-icons-sharp'>drive_file_rename_outline</span>
                                </a></td>
                                <td><a href='delete_scanner.php?id=" . $row['id'] . "'>
        ...
        <section class="right">
            <div class="investments">
                <div class="header">
                    ...
                    <a href="../../application/controllers/user/search/Search_scanner.php">เพิ่มเติม<span class="material-icons-sharp">
                            chevron_right
                        </span></a>
                </div>
